...
public class SwarmBeaconSkill {
    ...
    private static LivingEntity getLowestArmorPlayer(com.nhatbh.basedefensev2.boss.skills.SkillContext ctx) {
        return ctx.boss().level().getEntitiesOfClass(net.minecraft.world.entity.player.Player.class, ctx.boss().getBoundingBox().inflate(64)).stream()
    ...
    private static void fireBeaconArrow(LivingEntity shooter, LivingEntity target) {
        Vec3 dir = target.position().add(0, target.getEyeHeight() * 0.5, 0).subtract(shooter.position().add(0, shooter.getEyeHeight(), 0)).normalize();
    ...
    private static void spawnLeaper(LivingEntity boss, LivingEntity target) {
        if (boss.level() instanceof ServerLevel level) {
            ...
            leaper.setPos(target.getX() + (Math.random() - 0.5) * 6, target.getY() + 2, target.getZ() + (Math.random() - 0.5) * 6);
            ...
            if (damageAttr != null) damageAttr.setBaseValue(2f);
