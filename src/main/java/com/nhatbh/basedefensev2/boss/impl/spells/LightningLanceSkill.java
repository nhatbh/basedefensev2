 ...
 * Mechanics: Fires a barrage of Lightning Lances at the lowest HP target's area with spread accuracy,
 * creating a lethal rain of lances that players can dodge.
 * 
 * Damage: Hybrid 15% Max HP total (7.5% Max HP + flat damage equal to 7.5% of 30 HP = 2.25 flat).
 */
    ...
    public static ActiveSequence create() {
        return ActiveSequence.builder("lightning_lance")
                ...
                .onTick(ctx -> {
                    ...
                    if (tick % 4 != 0) return; // Fire every 4 ticks

                    LivingEntity boss = ctx.boss();
                    Vec3 baseTargetPos = (Vec3) ctx.data().get("target_pos");
                    if (baseTargetPos == null) baseTargetPos = boss.position();
                    ...
                    // Origin position: Midpoint between boss and impact target, elevated 14 blocks up in the sky for an angled rain trajectory
                    ...
                    BossSpellCaster.castSpellFromLocation(boss, originPos, impactPos, SPELL_ID, FLAT_DMG_PER_LANCE, MAX_HP_PCT_PER_LANCE);
                })
