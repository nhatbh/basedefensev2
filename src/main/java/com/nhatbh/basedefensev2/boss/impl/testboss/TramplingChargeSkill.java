...
public class TramplingChargeSkill {
    public static ActiveSequence create() {
        return ActiveSequence.builder("trampling_charge")
            ...
            .step("rear_up", 15)
                .onStart(ctx -> {
                    ctx.log("§6Preparing Trampling Charge!§r");
                    ctx.boss().level().playSound(null, ctx.boss().getX(), ctx.boss().getY(), ctx.boss().getZ(), SoundEvents.HORSE_ANGRY, SoundSource.HOSTILE, 2.0f, 1.0f);
            ...
            .step("charge", 25)
                .onStart(ctx -> {
                    ctx.log("§lTRAMPLE!§r");
                    ctx.boss().level().playSound(null, ctx.boss().getX(), ctx.boss().getY(), ctx.boss().getZ(), SoundEvents.HORSE_GALLOP, SoundSource.HOSTILE, 2.0f, 1.5f);
                ...
                .onTick(ctx -> {
                    ...
                    if (dir != null) {
                        ...
                        List<LivingEntity> targets = HitboxUtils.getEntitiesInCircle(ctx.boss().level(), LivingEntity.class, ctx.boss().position(), 2.5, 
                            e -> e.isAlive() && e != ctx.boss() && e != ctx.boss().getVehicle() && !hitTargets.contains(e.getUUID()));
                        ...
                        for (LivingEntity target : targets) {
                            ...
                            ctx.boss().level().playSound(null, target.getX(), target.getY(), target.getZ(), SoundEvents.HORSE_STEP, SoundSource.HOSTILE, 1.5f, 1.0f);
                        }

                        if (ctx.boss().level() instanceof ServerLevel level) {
                            level.sendParticles(ParticleTypes.CRIT, ctx.boss().getX(), ctx.boss().getY(), ctx.boss().getZ(), 5, 0.5, 0.2, 0.5, 0.1);
                        }
                    }
                })
            ...
            .step("spear_strike", 15)
                .onStart(ctx -> {
                    ...
                    if (dir != null && ctx.boss().level() instanceof ServerLevel level) {
                        ...
                        ParticleUtils.renderArc(level, ParticleTypes.SWEEP_ATTACK, pos.add(0, 1, 0), dir, radius, angle, 12, 0.05);
                        
                        List<LivingEntity> targets = HitboxUtils.getEntitiesInArc(level, LivingEntity.class, pos, dir, radius, angle, 
                        ...
                        for (LivingEntity target : targets) {
                            ...
                            level.sendParticles(ParticleTypes.ENCHANTED_HIT, target.getX(), target.getY() + 1, target.getZ(), 10, 0.2, 0.2, 0.2, 0.2);
                        }
                    }
                })
