            ...
            .step("strike_windup", 45)
                ...
                .onTick(ctx -> {
                    if (ctx.getTicks() % 5 == 0 && ctx.boss().level() instanceof ServerLevel level) {
                        level.sendParticles(ParticleTypes.CRIT, ctx.boss().getX(), ctx.boss().getY() + 2, ctx.boss().getZ(), 5, 0.2, 0.2, 0.2, 0.1);
                        level.sendParticles(ParticleTypes.WITCH, ctx.boss().getX(), ctx.boss().getY() + 1, ctx.boss().getZ(), 5, 0.3, 0.3, 0.3, 0.05);
                    }
                })
                ...
                .onStart(ctx -> {
                    ...
                    if (target != null && target.isAlive()) {
                        ...
                        if (ctx.boss().level() instanceof ServerLevel level) {
                            level.sendParticles(ParticleTypes.ENCHANTED_HIT, target.getX(), target.getY() + 1, target.getZ(), 30, 0.5, 0.5, 0.5, 0.3);
                            level.sendParticles(ParticleTypes.SOUL, target.getX(), target.getY() + 1, target.getZ(), 20, 0.3, 0.3, 0.3, 0.1);
                        }

                        // Reset cooldown if killed
                        if (!target.isAlive()) {
                            BossComponent comp = BossManager.get(ctx.boss());
                            if (comp != null) comp.setSkillCooldown("culling_blade", 0);
                            ctx.boss().level().playSound(null, ctx.boss().getX(), ctx.boss().getY(), ctx.boss().getZ(),
