...
public class PartingShotSkill {
        ...
        return ActiveSequence.builder("parting_shot")
                ...
                .onStart(ctx -> {
                    ...
                    if (target != null) {
                        ...
                        BossSkillHelper.getMovementEntity(ctx).setDeltaMovement(dir.x, ctx.boss().getDeltaMovement().y,
                                dir.z);
                    }
                })
                ...
                .onStart(ctx -> {
                    ...
                    if (target != null) {
                        for (int i = 0; i < 3; i++) {
                            AreaEffectCloud trap = new AreaEffectCloud(ctx.boss().level(),
                                    target.getX() + (Math.random() - 0.5) * 4, target.getY(),
                                    target.getZ() + (Math.random() - 0.5) * 4);
                            trap.setRadius(2.5f);
                            trap.setDuration(120); // 6 seconds
                            trap.setParticle(ParticleTypes.SQUID_INK);
                            trap.addEffect(new net.minecraft.world.effect.MobEffectInstance(
                                    net.minecraft.world.effect.MobEffects.BLINDNESS, 60, 1));
                            trap.addEffect(new net.minecraft.world.effect.MobEffectInstance(
    ...
    private static void fireHeavyArrowVolley(LivingEntity shooter, LivingEntity target) {
        Vec3 baseDir = target.position().add(0, target.getEyeHeight() * 0.5, 0)
        ...
        for (int i = -2; i <= 2; i++) {
            ...
            BossSkillHelper.fireFastArrow(shooter, shooter.position().add(0, shooter.getEyeHeight(), 0), dir, 4.0f,
                    10.0);
        }
    }
}
