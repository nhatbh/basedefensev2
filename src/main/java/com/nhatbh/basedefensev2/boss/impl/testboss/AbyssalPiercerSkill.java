        ...
        return ActiveSequence.builder("abyssal_piercer")
            ...
            .step("charge", 40)
                ...
                .onTick(ctx -> {
                    ...
                    if (target != null) {
                         Vec3 dir = target.position().add(0, target.getEyeHeight() * 0.5, 0).subtract(ctx.boss().position().add(0, ctx.boss().getEyeHeight(), 0)).normalize();
                         ctx.data().put("piercer_dir", dir);
                         if (ctx.boss().level() instanceof ServerLevel level) {
                            level.sendParticles(ParticleTypes.WITCH, ctx.boss().getX(), ctx.boss().getY() + ctx.boss().getEyeHeight(), ctx.boss().getZ(), 5, 0.1, 0.1, 0.1, 0.05);
                         }
                    }
                })
                    ...
                    if (dir != null && ctx.boss().level() instanceof ServerLevel level) {
                        ...
                        for (float dist = 0; dist < 100; dist += 1.0f) {
                            ...
                            if (dist % 2 == 0) { // Optimize damage check density
                                level.getEntitiesOfClass(LivingEntity.class, new net.minecraft.world.phys.AABB(pos.subtract(1, 1, 1), pos.add(1, 1, 1))).forEach(e -> {
                                    if (e != ctx.boss()) {
