                ...
                .onTick(ctx -> {
                    if (ctx.getTicks() % 2 == 0 && ctx.boss().level() instanceof ServerLevel level) {
                        level.getEntitiesOfClass(net.minecraft.world.entity.player.Player.class,
                                ctx.boss().getBoundingBox().inflate(15.0)).forEach(p -> {
                                    ...
                                    if (ctx.getTicks() % 10 == 0) {
                                        ...
                                        p.hurt(ctx.boss().damageSources().indirectMagic(ctx.boss(), ctx.boss()),
                                                damage);
                                    }
                                });
                        com.nhatbh.basedefensev2.boss.utils.ParticleUtils.renderCircle(level,
                                ParticleTypes.REVERSE_PORTAL, ctx.boss().position(), 15.0, 20, 0.05);
                    }
                })
                ...
                .onStart(ctx -> {
                    ...
                    if (ctx.boss().level() instanceof ServerLevel level) {
                        ...
                        for (int i = 0; i < 6; i++) {
                            ...
                            clone.setPos(ctx.boss().getX() + (Math.random() - 0.5) * 8, ctx.boss().getY(),
                                    ctx.boss().getZ() + (Math.random() - 0.5) * 8);
                            ...
                            clone.setDropChance(net.minecraft.world.entity.EquipmentSlot.MAINHAND, 0); // Don't drop
                                                                                                       // boss weapon
                            ...
                            level.sendParticles(ParticleTypes.SOUL, clone.getX(), clone.getY() + 1, clone.getZ(), 10,
                                    0.2, 0.5, 0.2, 0.1);
                        }
                    }
                })
