...
public class SweepingPhalanxSkill {
    public static ActiveSequence create() {
        return ActiveSequence.builder("sweeping_phalanx")
            ...
            .step("lock", 15)
                .onStart(ctx -> {
                    ctx.log("§6Preparing Sweeping Phalanx!§r");
                    ctx.boss().level().playSound(null, ctx.boss().getX(), ctx.boss().getY(), ctx.boss().getZ(), SoundEvents.ARMOR_EQUIP_IRON, SoundSource.HOSTILE, 2.0f, 0.5f);
            ...
            .step("sweep_left", 10)
                .onStart(ctx -> {
                    ctx.log("§eLEFT TO RIGHT SWEEP!§r");
                    ctx.boss().level().playSound(null, ctx.boss().getX(), ctx.boss().getY(), ctx.boss().getZ(), SoundEvents.PLAYER_ATTACK_SWEEP, SoundSource.HOSTILE, 2.0f, 0.7f);
            ...
            .step("sweep_right", 10)
                .onStart(ctx -> {
                    ctx.log("§eRIGHT TO LEFT SWEEP!§r");
                    ctx.boss().level().playSound(null, ctx.boss().getX(), ctx.boss().getY(), ctx.boss().getZ(), SoundEvents.PLAYER_ATTACK_SWEEP, SoundSource.HOSTILE, 2.0f, 0.6f);
                    ctx.data().put("sweep_tick", 0);
                    ctx.data().put("sweep_hits", new java.util.ArrayList<java.util.UUID>());
                })
                .onTick(ctx -> {
            ...
            .step("spin", 15)
                .onStart(ctx -> {
                    ctx.log("§c§lPHALANX SPIN!§r");
                    ctx.boss().level().playSound(null, ctx.boss().getX(), ctx.boss().getY(), ctx.boss().getZ(), SoundEvents.PLAYER_ATTACK_SWEEP, SoundSource.HOSTILE, 2.0f, 0.4f);
                    ctx.boss().level().playSound(null, ctx.boss().getX(), ctx.boss().getY(), ctx.boss().getZ(), SoundEvents.IRON_GOLEM_ATTACK, SoundSource.HOSTILE, 1.5f, 0.5f);
                    ctx.data().put("sweep_tick", 0);
                    ctx.data().put("sweep_hits", new java.util.ArrayList<java.util.UUID>());
                })
                .onTick(ctx -> {
    ...
    private static void performDynamicSweep(SkillContext ctx, double startAngleOffset, double endAngleOffset, double radius, float damage, int duration, boolean applyKnockback) {
        ...
        if (dir != null && ctx.boss().level() instanceof ServerLevel level) {
            ...
            ParticleUtils.renderFilledArc(level, ParticleTypes.SWEEP_ATTACK, pos.add(0, 1, 0), rotatedDir, radius, arcWidth, 3, 3, 0.05);
            
            @SuppressWarnings("unchecked")
            List<java.util.UUID> hitTargets = (List<java.util.UUID>) ctx.data().get("sweep_hits");
            
            List<LivingEntity> targets = HitboxUtils.getEntitiesInArc(level, LivingEntity.class, pos, rotatedDir, radius, arcWidth, e -> e.isAlive() && e != ctx.boss() && e != ctx.boss().getVehicle() && !hitTargets.contains(e.getUUID()));
